...
- Une [app registry `JaskSSO`](/docs/datasources/providers/microsoft) configurée sur Microsoft Entra et ses informations renseignées dans `Gestion des secrets` sur Jask. [(Suivre le guide ici pour créer JaskSSO)](/docs/datasources/providers/microsoft)
- Un compte Jask avec le [rôle administrateur](/docs/users_groups/roles)
...
Choisissez `Autorisations d'application` et sélectionnez dans la liste l'autorisation `Sites.Selected`
...
<Callout type="warning">
  ...
  **obligatoire** pour pouvoir **donner l'autorisation à Jask** d'accéder à des
  sites SharePoint de votre organisation via `JaskSSO`.
</Callout>
...
Rendez vous sur Jask. Cliquez sur `Réglages` dans le menu de gauche puis sur `Connexions et secrets`. Rentrez le nom `JaskAdmin`, sélectionnez `Connexion Sharepoint (Obsolète)` puis ajoutez les identifiants récupérés à l'étape 1 et 2.

Choisissez la visibilité en fonction de qui peut utiliser cette clé admin pour donner les droits à `JaskSSO`. Nous vous conseillons de ne la partager qu'aux administrateurs connectant les sites SharePoint. Cliquez sur `Valider`.
...
<Cards>
  <Cards.Card title="Créer un agent" href="/docs/assistants/create" />
  <Cards.Card
